...
class Notificator {
    var lastUpdates: Dictionary<String, RegionStats> = [:];
    var isEnabled = false;
    ...
    func handle(newStats: RegionStats) {
        if (isUpdated(nextStats: newStats) && isEnabled) {
            showNotification(stats: newStats);
        }
        
        lastUpdates[newStats.country] = newStats;
    }
}
